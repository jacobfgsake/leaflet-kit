...
// Leaflet Kit header / nav bar. Mirrors the canonical markup in COMPONENTS.md.
// Published to Figma via `figma connect publish` (see package.json scripts).
//
// State = context (what is BEHIND the bar), not the page theme.
// Size (lg/xs) is the 768px breakpoint, handled automatically in kit.css -> no class here.
// The CTA ships cta-btn--sm always; kit.css promotes it to default size >=768.

figma.connect('https://www.figma.com/design/eaPdzBQjxfakthQhIgQfL0/Leaflet-UI-Kit?node-id=84-706', {
  props: {
